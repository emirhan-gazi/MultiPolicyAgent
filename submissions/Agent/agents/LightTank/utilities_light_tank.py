...
def multi_forced_anchor(movement, obs, team, map_grid): # birden fazla truck için
    ...
    for i,ally in enumerate(allies):
        if len(light_tanks) == 0 or i>6:
            break
        if isinstance(light_tanks[0], np.int64):
            light_tanks = np.expand_dims(light_tanks, axis=0)
        for light_tank in light_tanks:
            if (ally == light_tank).all():
