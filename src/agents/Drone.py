...
from utilities_drone import multi_forced_anchor, necessary_obs, decode_location, multi_reward_shape, enemy_locs, ally_locs, getDistance, avoid_mountain

class Drone(BaseLearningAgentGym):
        ...
        for i in range(y_max):
            for j in range(x_max):
                if units[team][i][j]<6 and units[team][i][j] != 0:
                    ...
                    {   
                        'unit': units[team][i][j],
                        'tag': Drone.tagToString[units[team][i][j]],
                        'hp': hps[team][i][j],
                        'location': (i,j),
                        'load': load[team][i][j]
                    }
                ...
                if units[enemy_team][i][j]<6 and units[enemy_team][i][j] != 0:
                    ...
                    {   
                        ...
                        'tag': Drone.tagToString[units[enemy_team][i][j]],
    ...
    def just_decode_state(obs, team, enemy_team):
        state, _ = Drone._decode_state(obs, team, enemy_team)
        return state
    ...
    def just_take_action(action, raw_state, team, map_grid):
        ...
        movement = avoid_mountain(movement, raw_state, team, map_grid)
        if len(locations) > 0:
